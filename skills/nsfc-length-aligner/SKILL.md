...
`【待补 ID：说明】` 与 `【暂定 …】` 是 draft-first 流程留下的缺口锚点：

- **不得删除、改写、合并或移位**。压缩时删掉一个 `【待补 …】`，等于让待补事实静默消失并直接进入投稿稿——这是本 skill 最危险的一种"压缩"。
- 正文里还有 `【待补 …】` 时，**篇幅结论只能报为暂定**，并显式说明。挖空稿偏短，补齐真实项目、论文与平台信息后通常多占 1–2 页；此时下"还有余量"的判断会在补齐后立刻反转，把流程拖进"压缩 → 补事实 → 又超 → 再压缩"的来回。
- 缺口清单可用 `python3 skills/nsfc-full-pipeline/scripts/scan_gaps.py --project-dir <项目路径>` 获取。
