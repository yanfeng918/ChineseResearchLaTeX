...
\else
  \ifNSFCHasBundledFonts
    ...
    % 英文正文字体：系统装了 Times New Roman（如 macOS / Windows）就直接用，可自由使用英文斜体和粗体；
    % 否则（如多数 Linux 发行版）回退到公共包内置的 TimesNewRoman.ttf，用 AutoFake 补出粗体和斜体。
    \IfFontExistsTF{Times New Roman}{%
      \setmainfont[BoldFont=Times New Roman, AutoFakeBold=5]{Times New Roman}%
    }{%
      \setmainfont[Path={\NSFCResolvedFontsDir}, Extension=.ttf, AutoFakeBold=5, AutoFakeSlant=0.2]{TimesNewRoman}%
    }
