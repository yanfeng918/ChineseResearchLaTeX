...
- draft-first 缺口标记单列一类，不计入 P0/P1：`【待补 ID：说明】` 表示正文已写、只有事实待补，`【暂定 …】` 表示草稿值待确认。二者都**不是**未完成写作，不要按"未解决占位符"判定。但必须统计数量并在报告中单列——只要还有 `【待补 …】`，本稿即不可提交。真正的未写作占位（`\NSFCBlankPara`、`待填写`、`现有材料未列`）仍按 P0 处理。
...
6. 引用核查清单
7. 篇幅与结构分布
8. 缺口清单（`【待补 ID】` 按 ID 分组列出位置，`【暂定 …】` 单列）与可提交判定
9. 建议的最小修改路线图
10. 附录：复现信息
