...
7. 在逻辑和引用检查之后，必须执行 Tier2 宿主 AI 语义检查，由宿主 AI 自主规划术语、论证维度与专业可读性复核：定位长句层级过多、指代不清、缩写/新概念未界定、抽象名词堆叠、段内衔接缺失或不增加准确性的修饰。输出“位置/障碍/影响/保真改法”的表达建议；不把专业正文改成科普文，也不因术语密度或必要限定而强行改写。宿主 AI 不可用时必须明确标记 Tier2 未完成并转人工复核，不得默认为通过。
...
2. 检查科学问题—假设—证据—研究内容的闭环和引用 key；随后必须运行 Tier2，由宿主 AI 自主决定术语、论证维度和表达方面的发现，并把结果分成“事实问题、逻辑问题、表达建议”。
...
可读性发现归入“表达建议”，不作为事实错误；Tier2 是完整诊断的必选项。宿主 AI 不可用或调用失败时，必须在报告中标记“Tier2 未完成”，转人工复核后才能继续，不得把回退提示当作语义检查结论。
...
- `refs`、`wordcount`：独立的引用和字数工具；术语、论证维度与专业可读性必须由宿主 AI 在主流程的 Tier2 中自主规划。
- `diagnose`、`coach`、`review`：主流程必须包含 Tier2 语义检查；结果是建议，不是固定模板的通过门槛。宿主 AI 不可用时报告未完成状态并转人工复核。措辞风险由宿主 AI 按参考准则判断，脚本不做固定短语命中。
