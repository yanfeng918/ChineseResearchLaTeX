...
- 新增 `scripts/pipeline_state.py` 与 schema v2：支持旧断点幂等迁移、原子写回、阶段输入/输出指纹、中断后产物恢复、`main.tex` 变更失效和真实缺口对账。
- 新增 `tests/` 回归测试，覆盖活动正文解析、孤儿文件排除、事实源缺失、未登记 ID、未写作占位、旧断点迁移、旧事实阻塞转换、中断恢复、外部事实源指纹、输入漂移失效与两级就绪度。
- 新增三份按需读取的中文参考文档，分别承载断点/缺口策略、stage 00–07 和 stage 08–14 细则。

### Changed（变更）

- 主 `SKILL.md` 收敛为中文 169 行入口，并修正 00–14 实为 15 个阶段的计数。
- `scan_gaps.py` 默认仅扫描 `main.tex` 实际引用的正文，显式诊断模式才扫描全部正文文件；事实源缺失、未登记 ID 与遗留未写作占位现会阻止误判。
- 以 `hard_gaps_clear` 替代含混的 `submittable`，并新增 `body_pipeline_ready` / `submission_ready` 两级判定，避免把正文完成误报为整份申请书可提交。
- 文献数量阈值改为学科适配；CS/AI 保留默认量化参考，其他领域以论断覆盖和语义匹配为硬要求。
- stage 13 只做定点修复和受影响检查，编译统一收敛到 stage 14。
- 修正 stage 06 将 `docs/05_研究基础素材.md` 同时列为输入和输出的自依赖；stage 02 将 BibTeX 明确登记为产物，stage 01/05/06/07 将两层事实文件纳入输入指纹。

### Fixed（修复）

- 统一主 Skill、catalog 与 evals 的 draft-first 语义，清除“缺事实即问卷阻塞”的旧口径。
- 为已有缺口保护行为补齐下游 `nsfc-qc`、`nsfc-length-aligner` 与 `nsfc-humanization` 的补丁版本号。
...
- 压缩断点文件模板：00–14 共 15 个阶段的重复字段改用流式列表表达，模板体积由 209 行降至约 150 行，字段语义不变。
