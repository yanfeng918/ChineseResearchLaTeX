...
事实分两层，**按顺序读**（分层规则见 [docs/applicants/README.md](../../docs/applicants/README.md)，接入步骤见 [docs/README.md](docs/README.md)）：
...
- 状态为 **`已确认`** 的事实**直接使用，严禁再向用户询问**
- **`公开来源待终核`** 可作草稿素材，正式提交前需复核
- **`待本人确认`** / **`明确暂无`** 不得推断为既成事实
- 需要补充时按事实 ID **一次性集中问完**，不要逐条打断写作
- 项目专属事实**不得**回写进共用申请人文件；申请人稳定事实**不得**在项目事实库里重复
- 断点文件 `not_applicable` 中列出的章节**不写、不问**
