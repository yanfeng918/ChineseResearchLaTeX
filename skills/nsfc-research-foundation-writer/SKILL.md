...
- **写入落点：按角色解析，禁止按编号硬写**。必须先执行下节的落点解析，得到 `foundation`（研究基础，含“研究风险的应对措施”）与 `work_conditions`（工作条件）两个角色对应的真实文件。
- **禁止改动**：`main.tex`、`extraTex/@config.tex`、任何 `.cls/.sty`
- **核心目标**：用“证据链 + 条件对位 + 风险预案”回答评审的三个问题：你做过吗？你做得成吗？出问题你怎么兜底？

## 落点解析（写作前必做）

**绝不允许用 `extraTex/3.*.tex` 这类编号 glob 选择写入目标。** NSFC 模板不共用一套章节编号且互相重叠：三段式的 `3.1` 是「研究基础」，五段式（`NSFC_Local`）的 `3.1` 却是「不同类型国基情况」——属于「其他说明」里的声明章节。按编号写会把研究基础写进声明章节，**且编译不报错**。
...
3. **排除同属"研究基础"大节但不由本技能撰写的文件**：承担项目 / 完成国基项目 / 项目完成情况。它们由用户或其他流程按真实信息填写，本技能不得写入。
4. 磁盘上存在但未被 `main.tex` 引用的文件是孤儿/禁用章节，不写入、也不算待办。
5. 把解析结果与下表对照做合理性校验；解析不出或角色缺失时**停下询问用户**，不得猜测编号。

| 布局 | 示例项目 | 研究基础 | 工作条件 |
|---|---|---|---|
| `three-part` | `NSFC_General` / `NSFC_General_Clean` / `NSFC_Young` | `3.1.研究基础` | `3.2.工作条件` |
| `five-part` | `NSFC_Local` / `NSFC_Local_Clean` | `2.1.研究基础` | `2.2.工作条件` |

**跨章节引用同样按角色**：本技能要与「研究内容」「年度研究计划」对齐，这两章在两种布局下也不同名——三段式为 `2.1.研究内容` / `2.3.年度研究计划`，五段式为 `1.2.内容目标问题` / `1.5.研究计划`。读取时按角色定位，不要按编号找。

## 参数与输出模式（建议显式提供）

- `project_root`：标书项目根目录（如 `projects/NSFC_Young`）
- `output_mode`（默认 `apply`）：
  - `preview`：按角色输出可复制的 LaTeX 正文草稿（并标注每段应写入的目标文件路径），不写入文件
  - `apply`：仅写入「落点解析」得出的 `foundation` 与 `work_conditions` 两个角色文件，不触碰其他文件
...
1. 仅编辑「落点解析」得出的两个角色文件；**严禁用 `extraTex/3.*.tex` 等编号 glob 批量选择写入目标**；不得修改 `main.tex`、`extraTex/@config.tex`、任何 `.cls/.sty`；不得写入承担项目 / 完成国基项目 / 项目完成情况等需按真实信息填写的文件
...
### Q2: 我还没有写「研究内容」，可以直接写「研究基础」吗？
**A**: 不建议。「工作条件」需要与「研究内容」的关键任务对齐。建议先使用 `nsfc-research-content-writer` 完成研究内容。
