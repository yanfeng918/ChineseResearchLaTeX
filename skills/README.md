...
**状态**：🚧 开发中（版本见 `skills/nsfc-research-content-writer/config.yaml`）

**类型**：📝 日常

**功能**：面向 NSFC 标书正文 `（二）研究内容` 写作/重构，技术路线按"总—分"两层结构编排并与研究内容逐项对应，同步产出 `特色与创新` 与 `年度研究计划`

**使用场景**：需要把“研究问题→目标→内容→技术路线→验证口径”写清，并输出 `2.1/2.2/2.3` 三份一致闭环的文件；尤其适合技术路线写成了方法罗列、看不出对应哪项研究内容的稿子
...
技术路线：先写总体路线（起点/主链/各研究内容落位/依赖关系/目标闭合），再逐项写分路线（每项研究内容一条，含输入/方法与关键步骤/输出/验证口径/衔接），条数与序号须与研究内容严格对应
```

**技能特点**：
- 技术路线强制"总—分"两层结构，分路线条数与序号必须匹配研究内容，杜绝缺口与孤儿
- 写完用"研究内容 ↔ 分路线"映射表核对数量、序号、术语、依赖闭合四件事
- 创新点用"相对坐标系"表达，避免绝对化措辞
- 年度计划要求体现推进依赖关系，而非"基础/深入/总结"流水账
...
### 23. nsfc-full-pipeline - NSFC 标书全流程编排

**状态**：🚧 开发中（版本见 `skills/nsfc-full-pipeline/config.yaml`）

**类型**：📝 日常

**功能**：把选题、文献、科学问题、研究方案、正文写作、引用核查、篇幅对齐、去 AI 味、QC、模拟评审、P0/P1 定点修复与编译串成一条可断点续跑的流水线，自动调用已有的 NSFC / research 系列技能

**适用项目**：`NSFC_General`、`NSFC_General_Clean`、`NSFC_Young`、`NSFC_Local`、`NSFC_Local_Clean`（省级基金 `GDNSF_*`、`GXNSF_*` 章节体例不同，不在覆盖范围）

**使用场景**：
- 想从头跑完整标书流程，而不是逐个手动触发单点技能
- 上次跑到一半中断，想接着跑而不是推倒重来
- QC 或模拟评审已出报告，希望 AI 直接改稿而不是只给建议
...
不想逐步手动触发时，可直接用 **nsfc-full-pipeline** 编排上述环节，并在中断后续跑。
